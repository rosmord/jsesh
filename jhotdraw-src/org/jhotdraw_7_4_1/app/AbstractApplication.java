...
import org.jhotdraw_7_4_1.app.action.file.LoadRecentFileAction;
import org.jhotdraw_7_4_1.app.action.file.OpenRecentFileAction;
import org.jhotdraw_7_4_1.beans.*;
 ...
 */
public abstract class AbstractApplication extends AbstractBean implements
		Application {
	...
	protected void initLabels() {
		labels = ResourceBundleUtil.getBundle("org.jhotdraw_7_4_1.app.Labels");
	}
	...
	/**
	 * Adds a separator to the supplied menu. The separator will only be added,
	 * if additional items are added using addAction.
	 */
	...
	public void clearRecentURIs() {
		...
		java.util.List<URI> oldValue = (java.util.List<URI>) recentFiles
				.clone();
		recentFiles.clear();
		prefs.putInt("recentFileCount", recentFiles.size());
		firePropertyChange("recentFiles", Collections
				.unmodifiableList(oldValue), Collections
				.unmodifiableList(recentFiles));
	}
	...
	private class OpenRecentMenuHandler implements PropertyChangeListener,
		...
		protected void updateOpenRecentMenu() {
			if (openRecentMenu.getItemCount() > 0) {
				JMenuItem clearRecentFilesItem = (JMenuItem) openRecentMenu
						.getItem(openRecentMenu.getItemCount() - 1);
				openRecentMenu.remove(openRecentMenu.getItemCount() - 1);

				// Dispose the actions and the menu items that are currently in
				// the menu
				...
				if (getAction(view, LoadFileAction.ID) != null || //
					...
					for (URI f : getRecentURIs()) {
						LoadRecentFileAction action = new LoadRecentFileAction(
								AbstractApplication.this, view, f);
						openRecentMenu.add(action);
						openRecentActions.add(action);
					}
				} else {
					for (URI f : getRecentURIs()) {
						OpenRecentFileAction action = new OpenRecentFileAction(
								AbstractApplication.this, f);
						openRecentMenu.add(action);
						openRecentActions.add(action);
					}
				}
		...
		public void dispose() {
			...
			// Dispose the actions and the menu items that are currently in the
			// menu
			for (Action action : openRecentActions) {
				if (action instanceof Disposable) {
					((Disposable) action).dispose();
	...
	public URIChooser getOpenChooser(View v) {
		if (v == null) {
			...
			URIChooser chooser = (URIChooser) v.getComponent()
					.getClientProperty("openChooser");
			if (chooser == null) {
	...
	public URIChooser getImportChooser(View v) {
		if (v == null) {
			...
			URIChooser chooser = (URIChooser) v.getComponent()
					.getClientProperty("importChooser");
			if (chooser == null) {
	...
	public URIChooser getExportChooser(View v) {
		if (v == null) {
			...
			URIChooser chooser = (URIChooser) v.getComponent()
					.getClientProperty("exportChooser");
			if (chooser == null) {
