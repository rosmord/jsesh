...
public class EMFGraphics2D extends BaseGraphics2D {
	 ...
	 * @throws IOException
	 */
	public EMFGraphics2D(RandomAccessStream out, Dimension dims, String creator, String comment)
			throws IOException {
	 ...
	 * @throws IOException
	 */
	public EMFGraphics2D(RandomAccessStream out, long maxx, long maxy, String creator, String comment)
			throws IOException {
     ...
     * @throws java.io.IOException
     */
    private void initEMFGraphics2D(RandomAccessStream out, long maxx, long maxy, String creator, String comment)
			throws IOException {
...
// R0011: [018] EMR_SETBKMODE	(s=12)	{iMode(2=OPAQUE)}
		...
		try {
			...
			while (!iter.isDone()) {
				...
				case PathIterator.SEG_QUADTO: {
					...
					controls[0] = new EMFPoint(
							lastPoint.getX() + (2*(quadricControl.getX() - lastPoint.getX()))/3,
							lastPoint.getY() + (2*(quadricControl.getY() - lastPoint.getY()))/3
					);
					
					controls[1] = new EMFPoint(
							endControl.getX() + (2*(quadricControl.getX() - endControl.getX()))/3,
							endControl.getY() + (2*(quadricControl.getY() - endControl.getY()))/3
					);
	...
	private EMFPoint deviceCoords(Point2D p) {
		...
		return new EMFPoint((short) (res.getX() * deviceScale), (short) (res
				.getY() * deviceScale));
	}
