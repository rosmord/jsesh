...
/**
 * A file dialog class which is adapted to the actual system being used. (it
 * means JFileChooser on Windows/Linux and FileDialog on mac).
 *
 * The provided functionalities are not that great, which has consequences on
 * the suggested way of asking for files: if needed the file dialog should be
 * lauched from another dialog.
 *
 * @author rosmord
 */
public abstract class PortableFileDialog {
    ...
    /**
     * Sets the currently selected file. Will of course set the current
     * directory.
     */
    ...
    /**
     * Gets the selected file's potential parent directory. If there is a
     * selected file (or directory) returns its parent. Else, return the
     * directory where the selected file would be created or searched for.
     *
     * @return a directory.
     */
    ...
    /**
     * Sets the file filter to use for this dialog (iff there is only one file
     * filter to define).
     *
     * @param filter
     */
