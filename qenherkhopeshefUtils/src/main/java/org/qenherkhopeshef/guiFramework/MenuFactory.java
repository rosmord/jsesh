...
/**
 * Factory for building menus and menu bars according to a textual
 * specification.
 * 
 * Basically, one provides a file (usually a resource) describing the menus.
 * 
 * Here is a sample of file content.
 * 
 * <pre>
 * # This file uses the actions described in demo8n.properties
 * fileMenu
 * open
 * # A comment. Below, you see a separating line.
 * ---------
 * saveAsMenu
 * 	saveAs_HTML
 * 	saveAs_Text
 ...
 * 
 * <ul>
 * <li>The file should be an ASCII file. comments are introduced by "#".
 * <li>Names are the action identifiers used in the property files for the
 * action factory.
 * <li>Aside from that, indentation is the key: menu names should start in the
 * first column, actions in a menu in the second columns, etc.
 * <li>Sub-menus are easy to create.
 * <li>Any line with "--" is a separator.
 * </ul>
 * 
 * @author rosmord
 */
...
public class MenuFactory {
	...
	/**
	 * If we are working on a menu and not on a menuBar, the menu we are
	 * filling.
	 */
	...
	/**
	 * Create a menu factory with actions from a certain action map.
	 * 
	 * @param actions
	 */

	public MenuFactory(Map<String, Action> actions) {
	 ...
	 * @throws IOException
	 */
	public void fillMenuBar(JMenuBar menuBar, InputStreamReader reader)
			throws IOException {
		menuToFill = null;
	...
	private void processFile(InputStreamReader reader) throws IOException {
		...
		try {
			...
			throw new RuntimeException("Problem with line " + lineNumber
					+ " : " + line, e);
		} finally {
	 ...
	 * @throws IOException
	 */
	public void fillJMenu(JMenu menu, InputStreamReader reader)
			throws IOException {
		menuBar = null;
		menuToFill = menu;
		menusStack = new Stack<JMenu>();
	...
	private void addMenuLine(String line) {
		...
		if ("quitApplication".equals(actionName)
