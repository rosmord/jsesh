...
public class Main {
	...
	protected static void parse_args(String argv[]) {
		...
		for (i = 0; i < len; i++) {
			...
			} else if (argv[i].equals("-outputDir")) { // Added S. Rosmorduc
				/* must have an arg */
				if (++i >= len || argv[i].startsWith("-")
						|| argv[i].endsWith(".cup"))
					usage("-outputDir must have a name argument");
				outputDir= new File(argv[i]);
				if (! outputDir.isDirectory()) 
					usage("-outputDir must be an existing directory");
	...
	/**
	 * Output directory for created files. (Added S. Rosmorduc)
	 */
	...
	protected static void open_files() {
		...
		fil = new File(outputDir, out_name);
		try {
