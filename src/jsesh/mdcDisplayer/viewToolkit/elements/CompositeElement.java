...
import jsesh.mdcDisplayer.viewToolkit.drawing.event.CompositeEvent;
...
public class CompositeElement extends GraphicalElement {
	...
	public void setBounds(double x, double y, double width, double height) {
		...
		for (GraphicalElement elt : this) {
			...
			elt.setBounds(b.getMinX(), b.getMinY(), width, b.getHeight());
		}
		fixDecoratorBounds();
	...
	public void pack() {
		...
		double y = 0;
		for (GraphicalElement elt : this) {
			Dimension2D pref = elt.getPreferredSize();
			if (pref.getWidth() > w)
				w = pref.getWidth();
		}
		for (GraphicalElement elt : this) {
