 ...
 * 
 * It doesn't deal with scaling and the like, as scaling is provided by the graphics
 * environment.
 * 
 * <p>This interface should probably not contain methods like getHeightOfA1 or isKnown,
 * which is really very specific to hieroglyphs.
 * <p>The reason is historical: this interface was in the beginning only devoted to
 * glyphs. But now, the environment for hieroglyphs has been extended (to philological
 * markup and even text recently).
 * <p>
...
public interface HieroglyphsDrawer {
     ...
     * @param code
     *            the hieroglyph code in Manuel de Codage
    ...
    /**
     * Gets a sign bounding box, or null if ! isKnown(code).
     * The bounding box is the "natural" box for this sign.
     * In some cases, the sign will be larger.
     * 
     * <p> Note about "fixed" : a few signs (currently, mainly editor markup) may have 
     * two size: an elastic one (which is very small, but can grow), and a "natural" one,
     * which is fixed size, and used when the sign is more or less alone. Most signs won't care
     * about the value of fixed.  
     * @param code
     * @param angle the rotation angle applied to this sign (in degree, clockward).
    ...
    /**
     * Get the sign area.
     * The area of a sign is the region enclosed in its outer boundaries.
     * @param code
     * 
     * @param x
     * @param y
	 ...
	 * <p> Typical case : G1 sign, with two areas, one in the bottom-start corner of the sign, the other in the top-end corner.
	 * We might actually propose an half automated system.
	 * @param i the number of the area : 0 or 1 or 2
	 * @param code the code of the sign.
	 * @return a rectangle if an area is available, or null if none is.
	 */
	LigatureZone getLigatureZone(int i, String code);
	...
	/**
	 * Returns the length of 1 "group" unit.
	 * Groups units are relative to the size of the fonts. More precisely,
	 * they are 1/1000 of the height of the A1 sign. 
	 * @return
	 */
