...
public class OpenFileAction extends AbstractApplicationAction {
	...
	protected void openViewFromURI(final View view, final URI uri,
		...
		for (View aView : app.views()) {
			if (aView != view && aView.getURI() != null
				...
				multipleOpenId = Math.max(multipleOpenId, aView
						.getMultipleOpenId() + 1);
			}
		}
		...
		view.execute(new Worker() {
			...
			public Object construct() throws IOException {
				...
				if (exists) {
					...
					throw new IOException(labels.getFormatted(
							"file.open.fileDoesNotExist.message", URIUtil
									.getName(uri)));
				}
			}
			...
			protected void failed(Throwable value) {
				...
				ResourceBundleUtil labels = ResourceBundleUtil
						.getBundle("org.jhotdraw_7_4_1.app.Labels");
				JSheet.showMessageSheet(view.getComponent(), "<html>"
						+ UIManager.getString("OptionPane.css")
						+ "<b>"
						+ labels.getFormatted("file.open.couldntOpen.message",
	 ...
	 * dialogs properly on screen on Mac OS X.
	 */
	protected JDialog createDialog(URIChooser chooser, Component parent)
			throws HeadlessException {
		String title = chooser.getDialogTitle();
		if (chooser instanceof JFileChooser) {
			((JFileChooser) chooser).getAccessibleContext()
		...
		if (parent == null)
			...
			window = (parent instanceof Window) ? (Window) parent
		...
		dialog.setComponentOrientation(chooser.getComponent()
		...
		// dialog.pack();
		Preferences prefs = PreferencesUtil.userNodeForPackage(getApplication()
				.getModel().getClass());
