...
* Improved reactivity. Better experience when navigating through the text using the keyboard.
		Tech. detail : we used to change the drawing view and the MDC text window immediatly when the document was changed. The result was that the scrolling wasn't smooth when navigating using the keyboard arrows. Now, these changes are made using SwingUtilities.invokeLater(), and the scrolling is much better.
