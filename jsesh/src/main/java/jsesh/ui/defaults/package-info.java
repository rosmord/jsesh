 ...
 *
 * <ul>
 * <li>{@link jsesh.ui.defaults.PredefinedFonts} builds individual, self-contained
 *     fonts (standard JSesh font, GnuTrace font).
 * <li>{@link jsesh.ui.defaults.HieroglyphResourcesBuilder} composes fonts (order
 *     matters: first added is searched first) plus the sign database and the
 *     completion possibilities. It depends only on a plain
 *     {@link jsesh.utils.DirectoryHolder} for user fonts, so it stays pure and
 *     unit-testable with no persistence involved.
 * <li>{@link jsesh.ui.defaults.HieroglyphResources} is the immutable result handed
 *     to the rest of the application.
 * <li>{@link jsesh.ui.defaults.UserFontDirectoryManager} is the app-scoped adapter
 ...
 * {@link jsesh.ui.defaults.HieroglyphResourcesBuilder#buildFull(jsesh.utils.DirectoryHolder, jsesh.ui.glossary.Glossary)} directly with a bare
 * with a bare {@link jsesh.utils.DirectoryHolder} and a bare {@link jsesh.ui.glossary.Glossary}.
 */
package jsesh.ui.defaults;
