...
public class CompositeHieroglyphsManager implements HieroglyphDatabaseInterface {
	...
	private Map<String, PossibilitiesList> possibilityListsForKeyBoard = new HashMap<String, PossibilitiesList>();
	...
	public PossibilitiesList getPossibilityFor(String phoneticValue,
		...
		if (SignDescriptionConstants.KEYBOARD.equals(level)) {
			if (!possibilityListsForKeyBoard.containsKey(phoneticValue))
				possibilityListsForKeyBoard.put(phoneticValue,
						fetchPossibilityListFromBases(phoneticValue, level));
			result = possibilityListsForKeyBoard.get(phoneticValue);
		} else
		...
		if (GardinerCode.isCorrectGardinerCode(code)) {
			if (!possibilityListsForKeyBoard.containsKey(code)) {
				...
				PossibilitiesList p1 = userInfoManager
						.getSuitableSignsForCode(code);
				possibilityListsForKeyBoard.put(code, p.add(p1));
			}
			return possibilityListsForKeyBoard.get(code);
		} else {
