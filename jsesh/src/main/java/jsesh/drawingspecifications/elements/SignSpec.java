 ...
 *                           smaller than <code>getBaseLength() * getSmallSize(),</code> this
 *                           sign is small.
 * @param smallBodyScaleLimit the size (in points) below which we will start using
 *                           small body fonts. For instance, if we set it to 10,
 *                           any sign drawn with a scale such that the A1 sign
 *                           height would be below the said limit will be drawn in
...
public record SignSpec(
	...
	public static class Builder {
		...
		public SignSpec build() {
			return new SignSpec(largeSignSizeRatio, smallSignSizeRatio, smallBodyScaleLimit);
		}
	}
