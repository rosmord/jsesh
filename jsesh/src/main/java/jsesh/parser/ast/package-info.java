 ...
 *
 * <p>Unlike {@link jsesh.model}, which is built by {@link jsesh.model.MDCModelBuilder}
 * and already applies some interpretation while parsing (e.g. red/shaded state is
 * folded into each item instead of kept as toggle markers, some constructs such as
 * cadrat and zone options are dropped altogether), this package keeps a faithful,
 * uninterpreted record of what the parser actually read: every construct recognized
 * by the grammar becomes a node, in the order it was parsed.
 *
