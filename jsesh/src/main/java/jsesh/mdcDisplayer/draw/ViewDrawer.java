...
public class ViewDrawer {
	...
	REMOVED : it belongs in the document view, which should be created as a class.
	WeakHashMap<TopItem, MDCView> viewMap= new  WeakHashMap<TopItem, MDCView>();
*/
	 ...
	 * 
	 * @param g
	 *            a graphics2D, whose origin should be v's top left corner.
	 * @param i
	 *            the position in the current TopItemList
	 * @param x
	 ...
	 * @param view
	 * @param cursor
	 *            : the cursor position. If it's null, the cursor is not drawn.
	 * @param ds
	 * @param start
	 * @param end
	 */
	 ...
	 * 
	 * @param v
	 *            a view for a <em>TopItemList</em> (this might and should
	 *            change ?)
	 * @param clickPoint
	 *            : a point, its coordinates expressed in display space. Note
	 *            that in the case of JMDCDisplayer, display space scaling
	 *            should be applied before calling this function, as the drawer
	 *            knows nothing of it.
	 * @param ds
	...
	/**
	 * Request or prevent image and view caching. Image caching is only possible for
	 * raster output. In practice, its only use is for interactive displays of
	 * hieroglyphs.
	 * 
	 * View caching is needed for caret to be found and interactive edition in general. 
	 * We will probably separate the two issues.
	 * 
	 * @param c
	 */
	...
	/**
	 * Draws a gray rectangle over a view.
	 * 
	 * @param g
	 * @param v
	 * 
	 ...
	 *            g.setPaint(drawingSpecifications.getGrayColor());
	 * 
	 *            // Note that we used to take the view's dx and dy into
	 *            account, but this // is done // even before drawing the view.
	 *            g.fill(new Rectangle2D.Float(0, 0, v.getWidth(),
	 *            v.getHeight()));
	 * 
	 *            // Fill the gap between this view and the next one, if
	 *            possible and // necessary.
	 * 
	 *            // We fill gaps between two adjacent cadrats. if
	 *            (drawingSpecifications.getTextOrientation().isHorizontal() &&
	 *            v.nextIsHorizontallyAdjacent()) { Rectangle2D.Double rect =
	 *            new Rectangle2D.Double(); // Shade the inter-cadrat space
	 *            rect.x = v.getWidth(); rect.y = 0f; rect.height =
	 *            v.getHeight(); rect.width = (float)
	 *            (v.getNext().getPosition().x - (v .getPosition().x +
	 *            v.getWidth())); g.fill(rect); }
	 * 
	 *            g.setPaint(oldPaint);
	 * 
	 *            }
				...
				if (drawAfterView) {
					...
					if (textOrientation.isHorizontal()) {
						if (textDirection.isLeftToRight())
							g
									.draw(new Double(v.getWidth(), 0, v
											.getWidth(), drawingSpecifications
											.getMaxCadratHeight()));
						else
							g.draw(new Double(0, 0, 0, drawingSpecifications
									.getMaxCadratHeight()));
					} else {
						g.draw(new Double(0, v.getHeight(),
					...
					if (textOrientation.isHorizontal()) {
						...
						g.draw(new Double(0, 0, drawingSpecifications
								.getMaxCadratWidth(), 0));
